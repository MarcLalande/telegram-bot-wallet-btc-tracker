...
WALLET_TO_TRACK=['bc1q0unygz3ddt8x0v33s6ztxkrnw0s0tl7zk4yxwd','1Ay8vMC7R1UbyCCZRVULMV7iQpHSAbguJP']
...
time_daily = datetime.time(hour=00, minute=00, second=00).replace(tzinfo=local_timezone)
import asyncio
    ...
    if response.status_code == 200:
        ...
        if previous_balance != new_balance:
            ...
            if operation >10: 
                ...
                await bot.send_message(chat_id=-1002249261036, text= f'Mr 100 has bought : {abs(previous_balance - new_balance)} BTC')

            elif  operation < -10: 
                negative_transactions.append(operation)
                await bot.send_message(chat_id=-1002249261036, text= f' Mr 100 has sold : {abs(previous_balance - new_balance)} BTC')
...
async def monitor_mr_100_command(update:Update,context:ContextTypes.DEFAULT_TYPE):
...
async def daily_count(context: ContextTypes.DEFAULT_TYPE) -> None:
   ...
   global negative_transactions
   global positive_transactions
   await bot.send_message(chat_id=-1002249261036, text= f' Mr 100 has bought : {sum(positive_transactions)} and sold : {sum(negative_transactions)} BTC .  the result is {sum(positive_transactions)-sum(negative_transactions)}')
   negative_transactions = []
   positive_transactions = []
